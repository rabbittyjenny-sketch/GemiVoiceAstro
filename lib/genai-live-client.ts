...
// FIX: Refactor to use composition over inheritance for EventEmitter to resolve issues with event handling methods.
export class GenAILiveClient {
  private emitter = new EventEmitter<LiveClientEventTypes>();

  // FIX: Expose on/off methods for event handling.
  ...
  public send(parts: Part | Part[], turnComplete: boolean = true) {
    if (this._status !== 'connected' || !this.session) {
      // FIX: Use the internal emitter instance to emit events.
