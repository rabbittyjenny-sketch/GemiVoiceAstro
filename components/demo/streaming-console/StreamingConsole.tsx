...
export default function StreamingConsole() {
  ...
  const { isFormSubmitted, userData } = useUserData();
  ...
  useEffect(() => {
    ...
    const userDataPrompt = isFormSubmitted
    ? `\n\nนี่คือข้อมูลของผู้ใช้สำหรับการทำนาย:
- ชื่อ: ${userData.name}
- วันเกิด: ${userData.dob}
- เวลาเกิด: ${userData.tob}
- สถานที่เกิด: ${userData.pob}
เริ่มต้นการสนทนาด้วยการทักทายผู้ใช้ด้วยชื่อของพวกเขาและยืนยันข้อมูลที่ให้มา จากนั้นจึงเริ่มคำทำนายตามสคริปต์ "go green" ที่กำหนดไว้`
    : '';
    ...
    const config: any = {
      ...
      systemInstruction: systemPrompt + userDataPrompt,
      tools: enabledTools,
    };

    setConfig(config);
  }, [setConfig, systemPrompt, tools, voice, isFormSubmitted, userData]);
  ...
  return (
    <div className="transcription-container">
      ...
      {!isFormSubmitted || turns.length === 0 ? (
        <WelcomeScreen />
      ) : (
